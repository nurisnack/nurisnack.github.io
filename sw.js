...
messaging.onBackgroundMessage(function(payload) {
  ...
  // Tag berbeda per jenis:
  // - pesanan baru (admin)  → 'nuri-admin-order'  (satu notif, replace tiap order baru)
  // - update status (pelanggan) → 'order-{orderId}' (replace per pesanan)
  // - lainnya (stok dll)    → 'nuri-notif'
  var tag;
  if (data.type === 'new_order') {
    tag = 'nuri-admin-order';
  } else if (data.orderId) {
    tag = 'order-' + data.orderId;  // ← satu notif per pesanan, selalu diganti
  } else {
    tag = 'nuri-notif';
  }
